...
ターミナルで note のメールアドレスとパスワードを聞かれます。パスワードは伏せ字 `*`
で表示され、保存はされません（その場でブラウザの入力欄に入れるだけです）。
空のまま Enter を押した場合は聞き直します。
...
note が「私はロボットではありません」などの認証を出すことがあります。これは
本人が通す必要があるため、ログイン用のブラウザは画面いっぱいに開きます。
出ている場合はターミナルにも案内が出るので、ブラウザの窓で認証を済ませてください。
認証が終わると自動で先に進みます。
